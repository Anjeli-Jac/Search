...
public class Terrain {
    ...
    /**
     * Place tokens on the terrain to indicate the path path
     * @param path A search algorithm generated path.
     * @throws TerrainFileException
     * @throws TerrainBoundsException
     */
    public void applyPath(Traversable<Direction> path) {
        ...
        path.reset();
        while(!current.equals(goal) && path.hasNext()) {

            Direction previousTo = to;
            to = path.next();

            // ingnore bad directions
            ...
            // ignore any bad move.
            if(!inTerrain(next) || isWall(next)) {
                System.err.println(String.format("ERROR: can't move %s from %s to %s. Ignoring...", to, current, next));
                continue;
            }

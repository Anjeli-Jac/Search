...
public class Main {
    ...
    public static void main(String[] args) throws IOException, UnknownAlgorithm {
        ...
        while(menu.nextChoice()) {
            ...
            Traversable<Direction> path = search.solve(terrain.getStart());

            if(search.lastSearchFoundSolution())
                System.out.println("Search has seen the goal.");
            else
                System.out.println("Search has not seen the goal.");

            // output solution
            terrain.applyPath(path);
            ...
            // animate if setup
            if(config.animate())
                GraphicalTerrain.run(terrain, search, path, WINDOW_WIDTH, WINDOW_HEIGHT, false, config.getPlayer());
